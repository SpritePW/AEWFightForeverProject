...
AELSSGameplayCamera::AELSSGameplayCamera(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer) {
    ...
    this->Camera->SetupAttachment(CameraParent);
    this->CameraParent->SetupAttachment(SpringArm);
    this->SpringArm->SetupAttachment(DefaultSceneRoot);
}
